        ...
        <div className="flex flex-col items-center pt-24">
            ...
            <div>
                {undeliveredDeliveries.length > 0 ? (
                    <div className="flex flex-col sm:flex-row gap-12 items-center py-24 justify-center">
                        {undeliveredDeliveries.map((delivery) => {
                            const { date, time } = formatScheduledDate(delivery.scheduledAt);

                            return (
                                <div
                                    className="flex flex-col bg-slate-100 text-slate-800 border-2 border-slate-200 gap-3 shadow-md rounded-md w-80 h-[250px] p-6"
                                    key={delivery.id}
                                >
